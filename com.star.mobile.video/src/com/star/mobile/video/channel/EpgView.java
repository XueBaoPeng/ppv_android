...
//	}
	private void initView() {
		...
		lv_epg_list.setLoadingListener(new LoadingListener<ProgramVO>() {
			
			@Override
			public List<ProgramVO> loadingS(int offset, int requestCount) {
				return getProgramService().getEpgs(mChannel.getId(), currentTime, offset,requestCount);
			}

			@Override
			public void loadPost(List<ProgramVO> datas) {
				mHasLoadedOnce = true;
...
//						lv_epg_list.setSelection(1);
			...
			@Override
			public List<ProgramVO> loadingL(int offset, int requestCount) {
				try {
					Thread.sleep(300);
			...
			@Override
			public List<ProgramVO> getFillList() {
				return epgs;
			}
