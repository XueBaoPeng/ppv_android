...
//					}
	...
	public void onGlobalLayout() {
		if (once) {
			...
			float scale = 1.0f;
			float scale_vw = (getWidth() * 1.0f) /dw;
			float scale_vh = (getHeight() * 1.0f) /dh;
			...
			}else if (dw <= getWidth() - mHorizontalPadding * 2
					&& dh <= getHeight() - mVerticalPadding * 2) {
...
//			SCALE_MID = initScale * 2;
			...
			float scale_v = Math.min(scale_vw, scale_vh);
			mScaleMatrix.postTranslate((width - dw) / 2, (height - dh) / 2);
			if(scale>2 || scale>scale_v){
				mScaleMatrix.postScale(scale, scale, getWidth() / 2, getHeight() / 2);
			}else{
				mScaleMatrix.postScale(scale_v, scale_v, getWidth() / 2, getHeight() / 2);
			}
			// 图片移动至屏幕中心
			setImageMatrix(mScaleMatrix);
			once = false;
	...
	public Bitmap clip() {
		...
		draw(canvas);
		return BitmapUtil.scaleBitmap(BitmapUtil.toRoundBitmap(Bitmap.createBitmap(bitmap, mHorizontalPadding,
				mVerticalPadding, getWidth() - 2 * mHorizontalPadding,
				getWidth() - 2 * mHorizontalPadding)), 30);
	}
