...
public class EpgDetailActivity extends BaseActivity {
	...
	private ImageView iv_buy_ppv;
	private RatingBar ratingBar;
	private TextView tv_epg_price;
	private TextView tv_epg_duration;
	private LinearLayout ll_channel_rating;
	private LinearLayout ll_channel_price;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(ppvSharesPre.isFirstCome()){
			CommonUtil.getInstance().showPromptDialog(EpgDetailActivity.this, getString(R.string.ppv_promotion), getString(R.string.ppv_promotion_message), getString(R.string.ok),null, new  CommonUtil.PromptDialogClickListener() {
				@Override
	...
	private void initView() {
		...
		ll_channel_rating=(LinearLayout)findViewById(R.id.ll_channel_rating);
		ll_channel_price=(LinearLayout)findViewById(R.id.ll_channel_price);

		tv_epg_price=(TextView) findViewById(R.id.tv_epg_price);
		tv_epg_duration=(TextView) findViewById(R.id.tv_epg_duration);
		iv_buy_ppv = (ImageView) findViewById(R.id.iv_buy_ppv);
		...
		iv_buy_ppv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if(SharedPreferencesUtil.getUserName(EpgDetailActivity.this)!=null){
					loadSmartCard();
				}else{
					CommonUtil.getInstance().showPromptDialog(EpgDetailActivity.this, getString(R.string.tips), getString(R.string.ppv_buy_not_login), getString(R.string.login_big), getString(R.string.cancel)
							, new CommonUtil.PromptDialogClickListener() {
						@Override
						public void onConfirmClick() {
							Intent intent = new Intent(EpgDetailActivity.this, ChooseAreaActivity.class);
...
//		updateUI();
	...
	private void loadSmartCard(){
		mSmartcardService.getAllNetSmartCardInfos(new OnListResultListener<SmartCardInfoVO>() {
			...
			public void onSuccess(List<SmartCardInfoVO> smartCardInfos) {
				...
				if (smartCardInfos.size() > 0) {
					...
					CommonUtil.getInstance().showPromptDialog(EpgDetailActivity.this, getString(R.string.tips), getString(R.string.ppv_buy_not_bind_card), getString(R.string.bind_big), getString(R.string.cancel)
							, new CommonUtil.PromptDialogClickListener() {
						@Override
						public void onConfirmClick() {
							Intent intent = new Intent(EpgDetailActivity.this, SmartCardControlActivity.class);
							CommonUtil.startActivity(EpgDetailActivity.this, intent);
							finish();
						}
	...
	private void updateUI() {
		...
		if(program.getPpvContent()!=null){
			...
			tvEpgStartDate.setText(DateFormat.formatMonth(new Date(startDate)));
			tv_epg_startime.setText(startime.substring(0,startime.length()-1));
			iv_buy_ppv.setVisibility(View.VISIBLE);
			ppvContent=program.getPpvContent();
			if(ppvContent.getGrade()!=null&&!"".equals(ppvContent.getGrade())){
				ll_channel_rating.setVisibility(View.VISIBLE);
				ratingBar.setRating(Float.valueOf(ppvContent.getGrade()));
			}
			List<Product> product=ppvContent.getProductList();
//			for(int i=0;i<product.size();i++){
//				if(program.getCreateDate().getTime()==product.get(i).getLicenceStart()){
			if(product.size()>0) {
				ll_channel_price.setVisibility(View.VISIBLE);
				tv_epg_price.setText(product.get(0).getCashPrice());
				long duration = product.get(0).getEpgContentList().get(0).getEndDate()-product.get(0).getEpgContentList().get(0).getStartDate();
				tv_epg_duration.setText(String.valueOf((int) Math.floor(duration / (60 * 1000))) + "min");
			}
//				}
//			}
		}else{
			iv_buy_ppv.setVisibility(View.GONE);
			ll_channel_rating.setVisibility(View.GONE);
			ll_channel_price.setVisibility(View.GONE);
			try{
...
//			}
	...
	public void setCurrentProgram(ProgramPPV program) {
	...
	public void setCurrentProgram(final long programID,boolean isShowAlert) {
		programService.getEpgDetailByIdFromServer(programID, new OnResultListener<ProgramPPV>() {
			@Override
			public boolean onIntercept() {
				return false;
			}
			...
			@Override
			public void onSuccess(ProgramPPV value) {
				if(value!=null) if (program != null) {
					program.setDescription(value.getDescription());
					program.setContents(value.getContents());
					program.setCommentCount(value.getCommentCount());
					program.setSubhead(value.getSubhead());
					program.setClassification(value.getClassification());
					updateCommentCountForDB();
					if (!program.isIsFav())
						program.setFavCount(value.getFavCount());
				} else {
					program = value;
...
//		}.execute();
	...
	public void setCurrentProgram(final Content content) {
		long startTime =0;
		int channelNumber=0;
		for(int i=0;i<content.getProductList().size();i++){
			if(content.getProductList().get(i).getRentleType().equals(RentleType.SINGLE)){
				if(content.getProductList().get(i).getEpgContentList().get(0).getEndDate()-System.currentTimeMillis()>0){
					startTime=content.getProductList().get(i).getEpgContentList().get(0).getStartDate();
					channelNumber= Integer.parseInt(String.valueOf(content.getProductList().get(i).getEpgContentList().get(0).getLiveContent().getServiceId()));
					break;
				}
			}
		}
		programService.getEpgDetailByIdFromServer(channelNumber,startTime, new OnResultListener<ProgramPPV>() {
			@Override
			public boolean onIntercept() {
				return false;
			}
