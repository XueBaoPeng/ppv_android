...
public class NewVideoAdapter extends BaseAdapter {
	...
	public NewVideoAdapter(Context context, List<VOD> mRecommentsVideo) {
	...
	public int getCount() {
		...
		return (int) Math.ceil((double)(mRecommentsVideo.size())/2);
	}
	...
	public View getView(final int location, View convertView, final ViewGroup parent) {
		...
		if (mRecommentsVideo != null) {
			if(position*2 <= (mRecommentsVideo.size())){
				load_left(position*2);
				load_right(position*2+1);
			}else if(position*2 <= (mRecommentsVideo.size())){
				load_left(position*2);
