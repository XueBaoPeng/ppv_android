...
public class PlatformGridView extends LinearLayout implements
	...
	public void setHuidiaoCallback(TwitterShareCallback huidiaoCallback) {
	...
	public void onClick(View v) {
		...
		if (plat != null) {
			...
			if((plat instanceof FacebookCustomize) ) {
				FacebookCustomize fc = (FacebookCustomize)plat;
				fc.callback(colseCallback);
			}
			if((plat instanceof BbmCustomize)) {
				BbmCustomize bc=(BbmCustomize)plat;
				bc.callback(colseCallback);
			}
			if((plat instanceof TwitterCustomize)) {
				TwitterCustomize bc=(TwitterCustomize)plat;
				bc.callback(colseCallback);
				bc.hdcallback(huidiaoCallback);
			}
			if ((plat instanceof CustomPlatform)
				...
				shareData.put(plat, reqData);
				parent.share(shareData);
				if(name.equals("Email")||name.equals("ShortMessage")
						||name.equals("Wechat")||name.equals("WechatMoments")){
			...
			page.setHuiCallback(huidiaoCallback);
			page.setColseCallback(colseCallback);
			bgView = null;
	...
	private static class PlatformAdapter extends ViewPagerAdapter {
		...
		public PlatformAdapter(PlatformGridView platformGridView) {
			...
			if (platforms != null) {
				...
				Platform bbm=platforms[platforms.length-1];
				Platform facebook=platforms[platforms.length-2];
				Platform twitter=platforms[platforms.length-3];
				Platform first=platforms[1];
				Platform two=platforms[2];
				Platform three=platforms[3];
				platforms[1]=bbm;
				platforms[2]=facebook;
				platforms[3]=twitter;
				platforms[platforms.length-3]=first;
				platforms[platforms.length-2]=two;
				platforms[platforms.length-1]=three;
				logos.addAll(Arrays.asList(platforms));
			}
			ArrayList<CustomerLogo> customers = platformGridView.customers;
			if (customers != null) {
				logos.addAll(0,customers);
			}
