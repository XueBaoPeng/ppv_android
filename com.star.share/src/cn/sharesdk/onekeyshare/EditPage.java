...
public class EditPage extends FakeActivity implements OnClickListener {
	...
	private String platform_current;
	private ImageView iv_actionbar_back;//返回
	private TextView tv_actionbar_title;//标题
	private Button btn_cancel;//取消图片
	private LinearLayout at_linear; //@栏
	private Button at_btn;//@按钮
	private TextView at_title;//@朋友
	private Button share_btn;//分享
	// 文本编辑框
...
//	private LinearLayout llAt;
	...
	private TwitterShareCallback huiCallback;
	...
	public void setHuiCallback(TwitterShareCallback huiCallback) {
		this.huiCallback = huiCallback;
	}
	...
	public void onCreate() {
		...
		if(Twitter.NAME.equals(reqData.get("platform"))){
			if(huiCallback != null) {
				huiCallback.hui(twitterShareContent);
...
//						platform_current=name;
//						if(platform_current.equals(Twitter.NAME)){
//							Toast.makeText(context, "ddd", 2000).show();
//							}
						continue;
					}
					list.add(plat);
...
//	public void huidiao(HuidiaoCallback huiCallback) {
//		  huiCallback.hui("Twitter"); 
//	}
	public interface TwitterShareCallback { 
	        public void hui(String str); 
	    } 
	public interface ColseFriendCallback { 
        public void colse(String str); 
    } 
//	private void initView() {
//		iv_actionbar_back=(ImageView)findViewById(R.id.iv_actionbar_back);
//		tv_actionbar_title=(TextView)findViewById(R.id.tv_actionbar_title);
//		btn_cancel=(Button)findViewById(R.id.btn_cancel);
//		at_linear=(LinearLayout)findViewById(R.id.at_linear);
...
//			rlPage.addView(getImagePin());
	...
	private TitleLayout getPageTitle() {
		...
		if (resId > 0) {
			...
			llTitle.getBtnRight().setLayoutParams(params);
			llTitle.getBtnRight().setTextSize(TypedValue.COMPLEX_UNIT_DIP, 20);
		}
...
//		etContent.setGravity(Gravity.LEFT | Gravity.TOP);
		...
		if(reqData.get("platform").equals(Twitter.NAME)){
			etContent.setText(twitterShareContent);
		}else{
...
//		etContent.addTextChangedListener(this);
		...
		llMainBody.addView(getShareBottom());
...
//				ivPin.setVisibility(View.GONE);
		...
		lpBtn.setMargins(0, 15, 0, 0);
		btn.setLayoutParams(lpBtn);
		rlThumb.addView(btn);
//		rlThumb.addView(getImagePin());
		rlThumb.setVisibility(View.GONE);
		return rlThumb;
	}
...
//								ivPin.setVisibility(View.VISIBLE);
	...
	private LinearLayout getBodyBottom() {
		...
		llBottom.setGravity(Gravity.BOTTOM|Gravity.CENTER_HORIZONTAL);
...
//		llBottom.addView(tvCounter);
	...
	private LinearLayout getShareBottom() {
		LinearLayout lshareBottom = new LinearLayout(getContext());
		
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
		lshareBottom.setLayoutParams(params);
		share_btn = new Button(getContext());
		int dp_50 = dipToPx(getContext(), 50);
		LinearLayout.LayoutParams share_params = new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT,dp_50);
		share_params.setMargins(22,22,22,22);
		share_btn.setLayoutParams(share_params);
		int resId = getBitmapRes(activity, "share_btn_bg");
		if (resId > 0) {
			share_btn.setBackgroundResource(resId);
		}
		resId = getStringRes(activity, "share");
		if (resId > 0) {
			share_btn.setText(resId);
		}
		share_btn.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 22);
		share_btn.setTextColor(Color.WHITE);
		share_btn.setOnClickListener(this);
		lshareBottom.addView(share_btn);
		return lshareBottom;
	}
